...
describe Cat do
  ...
  subject(:nox) { described_class.new('Nox', 'Ragdoll') }
  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when Cat has method name shared with another class' do
    include_examples "shared method name"
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(nox).to_not be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(nox).to be_hiding
  end
end
