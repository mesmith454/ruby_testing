...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the previous step is 10 and the random number is 3'
  ...
  describe '#play' do
    ...
    context 'when the random number is 32' do
      ...
      subject(:game_thirtytwo) { described_class.new(32) }

      it 'will return 7' do
        result = game_thirtytwo.play
        expect(result).to eq(7)
      end
    end

    context 'when the random number is 100' do
      subject(:game_hundred) { described_class.new(32) }
      it 'will return 7' do
